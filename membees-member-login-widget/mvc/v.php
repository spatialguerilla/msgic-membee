...
function membee_login_shortcode( $atts ) {
  ...
  global $membee_options;
    ...
    if ($type == 'iframe') {
      ...
      $membee .= '<script src="https://memberservices.membee.com/feeds/Login/LoginFrameScript.ashx?clientid='.$membee_options['membee_client_id'].'&appid='.$membee_options['membee_app_id'].'&destURL='.urlencode('http://'.$_SERVER['HTTP_HOST'].urlencode($_SERVER['REQUEST_URI'])).'" type="text/javascript"></script>';
